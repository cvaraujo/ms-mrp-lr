...
double Model::arcsSelection() {
    ...
    // Select the arcs (s, s')
    tie(e, found) = edge(graph->getRoot(), 0, graphEdmonds);
    if (found) objective += get(weightMap, e);
...
double Model::cshpTerminal1Res(int k) {
    ...
    if (!r) {
        SPPRC_Graph_Vert_1_Res &vert_prop = get(vertex_bundle, cshpGraph1Res[k])[k];
        if (relaxNum <= 2) vert_prop.con = graph->getBigMDelay();
        else vert_prop.con = graph->getBigMJitter();   
        ...
        if (relaxNum == 1 || relaxNum == 2) vert_prop.con = graph->getParamDelay();
        else vert_prop.con = graph->getParamJitter();
        ...
        // if the first cshp doesnt returned value or the value are greater than shp 
    ...
    // If the Bellman Ford algorithm doesnt find a negative cycle
    } else {
    ...
    // if any path after the first cshp are less, return the first path computed
    for (auto p : firstPath)
       f[p.first][p.second][k] = true;
    return minPath;
}

double Model::cshpTerminal2Res(int k) {
    int actual, indexMin, i, j, l, root = graph->getRoot();
    double minPath, minSP, objective;
    vector<pair<int, int>> firstPath = vector<pair<int, int>>();
	...
	cout << parent[i] << " - " << i << endl;
	  ...
	  for (int i = 0; i < graph->getN(); i++) {
	    SPPRC_Graph_Vert_2_Res &vert_prop_aux = get(vertex_bundle, cshpGraph2Res[k])[i];
	    vert_prop_aux.con_1 = graph->getBigMDelay();
	    vert_prop_aux.con_2 = graph->getBigMJitter();
	  }
	  //	  cout << graph->removedF[root][0][k] << " - " << graph->removedF[0][k][k] << endl;
	  //	  cout << graph->getDelay(root, 0) << " - " << graph->getDelay(0, k) << endl;
	  // 	  cout << graph->getJitter(root, 0) << " - " << graph->getJitter(0, k) << endl;
	  //  getchar();
	    r_c_shortest_paths(cshpGraph2Res[k],
                         get(&SPPRC_Graph_Vert_2_Res::num, cshpGraph2Res[k]),
                         get(&SPPRC_Graph_Arc_2_Res::num, cshpGraph2Res[k]),
        ...
        vert_prop.con_1 = graph->getParamDelay();
        vert_prop.con_2 = graph->getParamJitter();
	//	vert_prop_aux.con_1 = graph->getParamDelay();
	//	vert_prop_aux.con_2 = graph->getParamJitter();
	...
	//cout << minSP << " - " << minPath << " - " << z[k] << endl;
	cout << "Terminal: " << k << ", " << minSP << endl;
	// if the first cshp doesnt returned value or the value are greater than shp 
        if (z[k] || minSP < minPath) {
	  //	  cout << "SP < minPath" << endl;
	  for (j = static_cast<int>(opt_solutions[indexMin].size())-1; j >= 0; --j) {
                i = source(opt_solutions[indexMin][j], cshpGraph2Res[k]);
                l = target(opt_solutions[indexMin][j], cshpGraph2Res[k]);
                f[i][l][k] = true;
...
void Model::updatePathCostsTerminals(int k, vector<double> &multipliersDelay, vector<double> &multipliersJitter, vector<vector<double>> &multipliersVar, vector<vector<double>> &multipliersLeaf, vector<vector<vector<double>>> &multipliersRel) {
    ...
    if (relaxNum == 1) {
        ...
        for (i = 1; i < n; i++) {
            for (auto *arc : graph->arcs[i]) {
                j = arc->getD();
                updateEdgePath(i, j, k, true, edgeCostAux * arc->getDelay() + 
                    multipliersJitter[k] * arc->getJitter() + multipliersRel[i][j][k]);
            }
        }
    } else if (relaxNum == 3) {
        ...
        for (auto *arc : graph->arcs[0]) {
            j = arc->getD();
            if (j != k) updateEdgePath(0, j, k, true, (edgeCostAux * arc->getDelay()) + multipliersRel[0][j][k] + multipliersLeaf[j][k]);
        }

        for (i = 1; i < n; i++) {
            for (auto *arc : graph->arcs[i]) {
                j = arc->getD();
                updateEdgePath(i, j, k, true, (edgeCostAux * arc->getDelay()) + multipliersRel[i][j][k]);
            }
        }
	if (k == 4) {
	  updateEdgePath(2, 4, 4, true, -1);
	  updateEdgePath(4, 10, 4, true, -1);
	  updateEdgePath(10, 1, 4, true, -1);
	  updateEdgePath(1, 4, 4, true, -1);
	}
    }   
}
  ...
  objectiveFunction = 0;
    ...
    for (auto k : graph->terminals) {
        if (graph->noPath[k]) {
            ...
            if (relaxNum <= 3) objectiveFunction += cshpTerminal1Res(k);
            else objectiveFunction += cshpTerminal2Res(k);
	   }
    }
