...
Model::Model(Graph *graph) {
    ...
    graphEdmonds = BoostGraph(graph->getNAfterRemoved());

    for(int i = 0; i < n; i++) 
	    add_vertex(SPPRC_Graph_Vert(i, graph->getParamDelay(), graph->getParamJitter()), CshpGraph);
    ...
    for (int i = 0; i < n; i++)
        for (auto arc : graph->arcs[i]) { 
	           add_edge(i, arc->getD(), SPPRC_Graph_Arc(countEdges++, 0.0, arc->getDelay(), arc->getJitter()), CshpGraph);
...
bool compareArcs(Arc_edmonds a, Arc_edmonds b) {
    return a.c < b.c;
}
...
void Model::edmonds() {
    ...
    vector<Arc_edmonds> arcsReturn = vector<Arc_edmonds>();
...
void Model::constrainedShortestpath(int k) {
    ...
    SPPRC_Graph_Vert &vert_prop = get(vertex_bundle, CshpGraph)[k];
    vert_prop.con_1 = graph->getParamDelay();
    vert_prop.con_2 = graph->getParamJitter();

    r_c_shortest_paths(CshpGraph,
                     get(&SPPRC_Graph_Vert::num, CshpGraph),
                     get(&SPPRC_Graph_Arc::num, CshpGraph),
                     graph->getRoot(),
                     k,
                     opt_solutions,
                     pareto_opt,
    ...
    for (int j = static_cast<int>(opt_solutions[indexMin].size())-1; j >= 0; --j) {
        f[source(opt_solutions[indexMin][j], CshpGraph)][target(opt_solutions[indexMin][j], CshpGraph)][k] = true;
        // cout << source(opt_solutions[indexMin][j], CshpGraph) << ", " << target(opt_solutions[indexMin][j], CshpGraph) << ", " << k << endl;
    }
...
void Model::updateEdgePath(int i, int j, double weight, bool increase) {
    edge_descriptor ed;
    bool found;
    tie(ed, found) = edge(i, j, CshpGraph);
    if (found) {
    	if (increase) {
    		SPPRC_Graph_Arc &arc_prop = get(edge_bundle, CshpGraph)[ed];
