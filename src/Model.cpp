...
bool operator==(const spp_spp_2_res_cont &res_cont_1, const spp_spp_2_res_cont &res_cont_2) {
    return (res_cont_1.cost == res_cont_2.cost && res_cont_1.res_1 == res_cont_2.res_1 &&
...
bool operator<(const spp_spp_2_res_cont &res_cont_1, const spp_spp_2_res_cont &res_cont_2) {
    ...
    if (res_cont_1.cost == res_cont_2.cost)
        return (res_cont_1.res_1 < res_cont_2.res_1 && res_cont_1.res_2 < res_cont_2.res_2);
    return true;
}
...
void Model::createGraphRL3() {
    ...
    for (int i = 0; i < n; i++) {
        for (auto arc : graph->arcs[i]) {
            ...
            for (auto k : graph->terminals) {
                if (!graph->removedF[i][arc->getD()][k]) {
                    add_edge(i, arc->getD(), SPPRC_Graph_Arc_1_Res(countEdges++, 0, arc->getJitter()),
                             cshpGraph1Res[k]);
                    add_edge(i, arc->getD(), 0.0, shpGraph[k]);
                }
            }
...
void Model::createGraphRL4() {
    ...
    for (int i = 0; i < n; i++) {
        for (auto arc : graph->arcs[i]) {
            ...
            for (auto k : graph->terminals) {
                if (!graph->removedF[i][arc->getD()][k]) {
                    add_edge(i, arc->getD(), SPPRC_Graph_Arc_2_Res(countEdges++, 0, arc->getDelay(), arc->getJitter()),
                             cshpGraph2Res[k]);
                    add_edge(i, arc->getD(), 0.0, shpGraph[k]);
                }
            }
...
double Model::shpTerminals(int k, vector<double> &multipliersDelay, vector<double> &multipliersJitter,
                           vector<vector<double>> &multipliersVar, vector<vector<vector<double>>> &multipliersRel) {
    int actual, i, j, l, root = graph->getRoot();
    double minPath, minSP, objective;

    vector<double> distance(graph->getN(), (numeric_limits<double>::max)());
...
double Model::cshpTerminal1Res(int k, vector<double> &multipliersDelay, vector<double> &multipliersJitter,
                               vector<vector<double>> &multipliersVar, vector<vector<vector<double>>> &multipliersRel) {
    int actual, indexMin = 0, i, j, l, root = graph->getRoot();
    double minPath = numeric_limits<int>::max(), minSP, objective;

    vector<vector<graph_traits<SPPRCGraph1Res>::edge_descriptor>> opt_solutions, opt_solutions2;
    vector<spp_spp_1_res_cont> pareto_opt, pareto_opt2;
    ...
    for (i = 0; i < graph->getN(); i++) {
        SPPRC_Graph_Vert_1_Res &vert_prop2 = get(vertex_bundle, cshpGraph1Res[k])[i];
        if (relaxNum == 2) vert_prop2.con = graph->getParamDelay() + 1;
        else vert_prop2.con = graph->getParamJitter() + 1;
    }
    ...
    double minPath2 = pareto_opt2[0].cost;
    int indexMin2 = 0;
    for (i = 0; i < opt_solutions2.size(); i++) {
        if (pareto_opt2[i].cost < minPath2) {
            minPath2 = pareto_opt2[i].cost;
            indexMin2 = i;
        }
    }
    ...
    for (i = 0; i < graph->getN(); i++) {
        SPPRC_Graph_Vert_1_Res &vert_prop2 = get(vertex_bundle, cshpGraph1Res[k])[i];
        if (relaxNum == 2) vert_prop2.con = graph->getParamDelay();
        else vert_prop2.con = graph->getParamJitter();
    }

    if (!pareto_opt.empty() && minPath < (minPath2 + 1)) {
        ...
        z[k] = true;
        for (j = static_cast<int>(opt_solutions2[indexMin2].size()) - 1; j >= 0; --j) {
            i = source(opt_solutions2[indexMin2][j], cshpGraph1Res[k]);
            l = target(opt_solutions2[indexMin2][j], cshpGraph1Res[k]);
            f[i][l][k] = true;
        }
        return minPath2;
    }
    ...
    if (!r) {
        ...
        if (!pareto_opt.empty()) {
            minPath = pareto_opt2[0].cost;
            indexMin = 0;
            for (i = 0; i < opt_solutions2.size(); i++) {
                if (pareto_opt2[i].cost < minPath) {
                    minPath = pareto_opt2[i].cost;
                    indexMin = i;
                }
            }
            // MinPath  = Param+1
            ...
            double minPath2;
            int indexMin2 = 0;
            minPath2 = pareto_opt[0].cost;
            for (i = 0; i < opt_solutions.size(); i++) {
                if (pareto_opt[i].cost < minPath2) {
                    minPath2 = pareto_opt[i].cost;
                    indexMin2 = i;
                }
            }

            if (minPath2 < (minPath + 1)) {
                for (j = static_cast<int>(opt_solutions[indexMin2].size()) - 1; j >= 0; --j) {
                    i = source(opt_solutions[indexMin2][j], cshpGraph1Res[k]);
                    l = target(opt_solutions[indexMin2][j], cshpGraph1Res[k]);
                    f[i][l][k] = true;
                }
                return minPath2;
            } else {
                z[k] = true;
                for (j = static_cast<int>(opt_solutions2[indexMin].size()) - 1; j >= 0; --j) {
                    i = source(opt_solutions2[indexMin][j], cshpGraph1Res[k]);
                    l = target(opt_solutions2[indexMin][j], cshpGraph1Res[k]);
                    f[i][l][k] = true;
                }
                return minPath;
            }
...
*/
    ...
    vector<vector<graph_traits<SPPRCGraph2Res>::edge_descriptor>> opt_solutions, opt_solutions2;
    vector<spp_spp_2_res_cont> pareto_opt, pareto_opt2;

    for (i = 0; i < graph->getN(); i++) {
        SPPRC_Graph_Vert_2_Res &vert_prop = get(vertex_bundle, cshpGraph2Res[k])[i];
    ...
    double minPath2 = numeric_limits<int>::max();
    int indexMin2 = 0;

    if (!pareto_opt2.empty()) {
        minPath2 = pareto_opt2[0].cost;
        indexMin2 = 0;
        for (i = 0; i < opt_solutions2.size(); i++) {
            if (pareto_opt2[i].cost < minPath2) {
                minPath2 = pareto_opt2[i].cost;
    ...
    if (!pareto_opt2.empty() && minPath2 < (minPath + 1)) {
        for (j = static_cast<int>(opt_solutions2[indexMin2].size()) - 1; j >= 0; --j) {
            i = source(opt_solutions2[indexMin2][j], cshpGraph2Res[k]);
            l = target(opt_solutions2[indexMin2][j], cshpGraph2Res[k]);
            f[i][l][k] = true;
        }
        return minPath2;
    } else {
...
double Model::penalty1Rl(vector<double> &multipliersDelay, vector<double> &multipliersJitter,
                         vector<vector<double>> &multipliersVar) {
    double penalty = 0, coefZ, objective = 0;
    int bigMK, bigML;

    for (auto k : graph->terminals) {
        coefZ = 1 - (multipliersDelay[k] * (graph->getParamDelay()+1)) + (multipliersJitter[k] * (graph->getParamJitter()+1));
        penalty -= (multipliersDelay[k] * graph->getParamDelay()) + (multipliersJitter[k] * graph->getParamJitter());
...
double Model::penalty2Rl(vector<double> &multipliersJitter, vector<vector<double>> &multipliersVar) {
    ...
    for (auto k : graph->terminals) {
        coefZ = 1 - (multipliersJitter[k] * (graph->getParamJitter()+1));
        penalty -= multipliersJitter[k] * graph->getParamJitter();
...
double Model::penalty3Rl(vector<double> &multipliersDelay, vector<vector<double>> &multipliersVar) {
    ...
    for (auto k : graph->terminals) {
        coefZ = 1 - (multipliersDelay[k] * (graph->getParamDelay()+1));
        penalty -= multipliersDelay[k] * graph->getParamDelay();
...
void Model::updatePathCostsTerminals(int k, vector<double> &multipliersDelay, vector<double> &multipliersJitter,
                                     vector<vector<double>> &multipliersVar,
   ...
   for (auto l : graph->terminals)
    ...
    if (relaxNum == 1) {
        for (i = 0; i < n; i++) {
            for (auto *arc : graph->arcs[i]) {
                j = arc->getD();
                updateEdgePath(i, j, k, true, multipliersDelay[k] * arc->getDelay() +
        ...
        for (i = 0; i < n; i++) {
            for (auto *arc : graph->arcs[i]) {
                j = arc->getD();
                updateEdgePath(i, j, k, true, edgeCostAux * arc->getDelay() + multipliersJitter[k] * arc->getJitter() +
                                              multipliersRel[i][j][k]);
            }
        }
    } else if (relaxNum == 3) {
        for (i = 0; i < n; i++) {
...
bool Model::solve(vector<double> &multipliersDelay, vector<double> &multipliersJitter,
                  vector<vector<double>> &multipliersVar, vector<vector<vector<double>>> &multipliersRel) {
    initialize();
    objectiveFunction = 0;
    ...
    for (auto k : graph->terminals) {
        ...
        if (graph->noPath[k]) {
            f[root][0][k] = true, f[0][k][k] = true;
            z[k] = true;
            if (relaxNum == 1) objectiveFunction = (getEdgeShp(root, 0, k) + getEdgeShp(0, k, k));
            else objectiveFunction += (getEdgeCshp(root, 0, k) + getEdgeCshp(0, k, k));
        } else {
            // Path to terminals
            updatePathCostsTerminals(k, multipliersDelay, multipliersJitter, multipliersVar, multipliersRel);
            if (relaxNum == 1)
                objectiveFunction += shpTerminals(k, multipliersDelay, multipliersJitter, multipliersVar,
                                                  multipliersRel);
            else if (relaxNum <= 3)
                objectiveFunction += cshpTerminal1Res(k, multipliersDelay, multipliersJitter, multipliersVar,
                                                      multipliersRel);
