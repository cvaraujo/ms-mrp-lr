...
Lagrangean::Lagrangean(Graph *graph, int relaxNum, bool heuristics, bool barrierMethod, double lambda, int maxIter,
                       int B, int time) {
...
double Lagrangean::solve() {
  ...
  while (iter < maxIter && endTime < time) {
    if (model->solve(multipliersDelay, multipliersJitter, multipliersVar, multipliersRel)) {
      ...
      for (int k : graph->terminals) {
        if (relaxNum == 1 || relaxNum == 3)
          multipliersDelay[k] = max(0.0, multipliersDelay[k] +
                                    gradientDelay[k] * thetaDelay);
        if (relaxNum <= 2)
          multipliersJitter[k] = max(0.0, multipliersJitter[k] + gradientJitter[k] * thetaJitter);
        for (int l : graph->terminals)
      ...
      for (auto k : graph->DuS)
        for (int i = 0; i < n; i++)
          for (auto *arc : graph->arcs[i])
            multipliersRel[i][arc->getD()][k] = max(0.0, multipliersRel[i][arc->getD()][k] +
                                                    (gradientRel[i][arc->getD()][k] * thetaRel));
